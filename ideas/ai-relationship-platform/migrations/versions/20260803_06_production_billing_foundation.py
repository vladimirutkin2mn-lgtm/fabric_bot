...
def upgrade() -> None:
    ...
    UPDATE payment_orders SET commercial_snapshot = jsonb_build_object('product_code', product_code, 'product_version', product_version, 'credits', credits, 'amount_minor', amount_minor, 'currency', currency, 'provider', provider, 'market', market, 'billing_period', billing_period);
    ...
    ALTER TABLE credit_transactions DROP CONSTRAINT ck_credit_transactions_type, DROP CONSTRAINT ck_credit_transactions_sign; ALTER TABLE credit_transactions ADD COLUMN original_purchase_transaction_id uuid REFERENCES credit_transactions(id) ON DELETE RESTRICT, ADD COLUMN refund_request_id uuid UNIQUE, ADD CONSTRAINT ck_credit_transactions_type CHECK(type IN ('grant','purchase','spend','refund','adjustment','purchase_refund')), ADD CONSTRAINT ck_credit_transactions_sign CHECK((type IN ('grant','purchase','refund') AND amount > 0) OR (type='purchase_refund' AND amount < 0) OR (type='spend' AND amount < 0) OR (type='adjustment' AND amount <> 0)), ADD CONSTRAINT fk_credit_transactions_refund_request FOREIGN KEY(refund_request_id) REFERENCES refund_requests(id) ON DELETE RESTRICT, ADD CONSTRAINT ck_credit_transactions_purchase_refund_refs CHECK(type <> 'purchase_refund' OR (original_purchase_transaction_id IS NOT NULL AND payment_order_id IS NOT NULL AND refund_request_id IS NOT NULL));
    """)


def downgrade() -> None:
    _execute_script("""
    ALTER TABLE credit_transactions DROP CONSTRAINT ck_credit_transactions_purchase_refund_refs, DROP CONSTRAINT fk_credit_transactions_refund_request, DROP COLUMN refund_request_id, DROP COLUMN original_purchase_transaction_id, DROP CONSTRAINT ck_credit_transactions_type, DROP CONSTRAINT ck_credit_transactions_sign, ADD CONSTRAINT ck_credit_transactions_type CHECK(type IN ('grant','purchase','spend','refund','adjustment')), ADD CONSTRAINT ck_credit_transactions_sign CHECK((type IN ('grant','purchase','refund') AND amount > 0) OR (type='spend' AND amount < 0) OR (type='adjustment' AND amount <> 0));
